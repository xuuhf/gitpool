...
<script type="text/javascript">
		function modifly(){
			...
			window.self.location = "ShowModiflyProduct?id=" + selectFlags[j].value;
		}
		...
		function deleteItem() {
			var selectFlags = document.getElementsByName("selectFlag");
				...
				if(window.confirm("你确定要删除吗?")){
					window.self.location = "DelProductServlet?id=" + selectFlags[j].value;
				}
		}
		function add(){
			window.self.location ="productlist.jsp";
		}
		
	</script>
<body>
<%
  PageModel<Product> pagemodel = (PageModel<Product>)request.getAttribute("product");
...
%>
		...
		<form action="/del" method="post">
			...
			<table>
				...
				<tr>
					<td>
					<input type="checkbox" name="selectFlag" value="<%=product.getId() %>" />  
					</td>
					<td id = "id">
						 <%=product.getId() %> 
					</td>
